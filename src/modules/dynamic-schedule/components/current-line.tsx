...
export const DynamicScheduleCurrentLine = (props: DynamicScheduleCurrentLineProps) => {
  ...
  const styles = {
    ...
    width:
      columnsQuantity && columnWidth
        ? `${columnsQuantity * columnWidth}px`
        : `calc(100% - ${firstColumnWidth}px)`,
  };
  ...
  return (
    <div
      className={cn("pointer-events-none absolute z-[1] h-[1px] bg-red-500", className)}
      style={styles}
    />
  );
};
